<?php

/**
 * Delete Grade Page
 * Admin Module
 */

$pageTitle = 'Delete Grade';
require_once __DIR__ . '/../../includes/admin-header.php';
require_once __DIR__ . '/../../config/database.php';

$gradeId = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);

if (!$gradeId) {
  $_SESSION['error'] = 'Invalid grade ID.';
  header('Location: view-grades.php');
  exit;
}

try {
  $stmt = $pdo->prepare("
        SELECT g.*, s.student_id as sid, s.name as student_name, c.course_code, c.course_name
        FROM grades g
        JOIN students s ON g.student_id = s.id
        JOIN courses c ON g.course_id = c.id
        WHERE g.id = :id
    ");
  $stmt->execute([':id' => $gradeId]);
  $grade = $stmt->fetch();

  if (!$grade) {
    $_SESSION['error'] = 'Grade not found.';
    header('Location: view-grades.php');
    exit;
  }
} catch (PDOException $e) {
  $_SESSION['error'] = 'Error loading grade.';
  header('Location: view-grades.php');
  exit;
}

$errors = [];

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!validateCsrfToken($_POST['csrf_token'] ?? '')) {
    $errors[] = 'Invalid request.';
  } else {
    if (!isset($_POST['confirm_delete'])) {
      $errors[] = 'Please confirm deletion.';
    }

    if (empty($errors)) {
      try {
        $stmt = $pdo->prepare("DELETE FROM grades WHERE id = :id");
        $stmt->execute([':id' => $gradeId]);
        $_SESSION['success'] = 'Grade deleted successfully.';
        header('Location: view-grades.php');
        exit;
      } catch (PDOException $e) {
        $errors[] = 'An error occurred while deleting the grade.';
      }
    }
  }
}
?>

<div class="row mb-4">
  <div class="col-12">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="view-grades.php">Grades</a></li>
        <li class="breadcrumb-item active">Delete Grade</li>
      </ol>
    </nav>
    <h1 class="mb-3 text-danger"><i class="bi bi-exclamation-triangle me-2"></i>Delete Grade</h1>
  </div>
</div>

<?php if (!empty($errors)): ?>
  <div class="alert alert-danger">
    <ul class="mb-0"><?php foreach ($errors as $e): ?><li><?= h($e) ?></li><?php endforeach; ?></ul>
  </div>
<?php endif; ?>

<div class="row">
  <div class="col-lg-6">
    <div class="alert alert-danger mb-4">
      <h5 class="alert-heading"><i class="bi bi-exclamation-circle me-2"></i>Warning!</h5>
      <p class="mb-0">You are about to permanently delete this grade record.</p>
    </div>

    <div class="card mb-4">
      <div class="card-header"><i class="bi bi-card-checklist me-2"></i>Grade Information</div>
      <div class="card-body">
        <table class="table table-borderless mb-0">
          <tr>
            <th>Student:</th>
            <td><?= h($grade['sid']) ?> - <?= h($grade['student_name']) ?></td>
          </tr>
          <tr>
            <th>Course:</th>
            <td><?= h($grade['course_code']) ?> - <?= h($grade['course_name']) ?></td>
          </tr>
          <tr>
            <th>Grade:</th>
            <td><span class="badge <?= getGradeClass($grade['grade']) ?>"><?= h($grade['grade']) ?></span></td>
          </tr>
          <tr>
            <th>Semester:</th>
            <td>Semester <?= h($grade['semester']) ?></td>
          </tr>
          <tr>
            <th>Academic Year:</th>
            <td><?= h($grade['academic_year']) ?></td>
          </tr>
        </table>
      </div>
    </div>

    <div class="card">
      <div class="card-body">
        <form method="POST" action="">
          <?= csrfInput() ?>
          <div class="form-check mb-4">
            <input class="form-check-input" type="checkbox" id="confirm_delete" name="confirm_delete" value="1">
            <label class="form-check-label" for="confirm_delete">
              I confirm that I want to delete this grade record permanently
            </label>
          </div>
          <div class="d-flex justify-content-between">
            <a href="view-grades.php" class="btn btn-outline-secondary"><i class="bi bi-arrow-left me-1"></i>Cancel</a>
            <button type="submit" class="btn btn-danger" id="delete-btn" disabled>
              <i class="bi bi-trash me-1"></i>Delete Grade
            </button>
          </div>
        </form>
      </div>
    </div>
  </div>
</div>

<script>
  document.getElementById('confirm_delete').addEventListener('change', function() {
    document.getElementById('delete-btn').disabled = !this.checked;
  });
</script>

<?php require_once __DIR__ . '/../../includes/admin-footer.php'; ?>