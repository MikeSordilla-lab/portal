...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!validateCsrfToken($_POST['csrf_token'] ?? '')) {
    ...
    if (empty($errors)) {
      try {
        $stmt = $pdo->prepare("
                    UPDATE schedule SET student_id = :sid, course_id = :cid, day_of_week = :day,
                    start_time = :start, end_time = :end, room_number = :room, 
                    instructor_name = :instructor, semester = :sem WHERE id = :id
                ");
        $stmt->execute([
        ...
        $_SESSION['success'] = 'Schedule entry updated successfully.';
        header('Location: view-schedule.php');
        exit;
      } catch (PDOException $e) {
        $errors[] = 'An error occurred while updating the schedule entry.';
      }
    }
  }
}
  ...
  <div class="col-12">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active">Edit Schedule Entry</li>
      </ol>
    </nav>
    <h1 class="mb-3"><i class="bi bi-pencil-square me-2"></i>Edit Schedule Entry</h1>
  </div>
</div>
